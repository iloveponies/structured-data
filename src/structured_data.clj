...
(defn second-elements [collection]
  (let [second-element (fn [foo] (get foo 1))]
...
(defn author->string [author]
  (let [years (str " (" (:birth-year author) " - " (:death-year author) ")")]
    (str (:name author) (if (not (nil? (get author :birth-year))) years ""))))
...
(defn books->string [books]
  (let [n (count books)]
    (if (== n 0) "No books."
      (str (str n " book" (if (> n 1) "s. " ". ")) (apply str (interpose ". " (map book->string books))) "."))))
