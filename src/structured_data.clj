...
(defn add-author [book new-author]
  (let [authors (:authors book)
        new-authors-group (conj authors new-author)]
    (assoc book :authors new-authors-group)))
...
(defn monotonic? [a-seq]
  (let [monotonic-by-fn (fn [fn'] (apply fn' a-seq))]
    (or
     (monotonic-by-fn <=)
     (monotonic-by-fn >=))))
...
(defn books->string [books]
  (cond
   ...
   (= 1 (count books)) (apply str
                            "1 book. "
                            (apply str (map str (map book->string books)))
                            ".")
   :else (apply str
                (count books)
                " books. "
