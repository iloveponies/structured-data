...
(defn books->string [books]
  (let [n (count books)
        lestring (fn [x] (apply str (interpose ". " (map book->string x))))]
    (cond
     (= 0 n) "No books."
     :else (str n " book" (if (= n 1) "." "s.") " " (lestring books) "."))))
...
(defn has-a-living-author? [book]
  (contains? (set (mapv alive? (:authors book))) true))
