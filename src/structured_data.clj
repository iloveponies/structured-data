...
(defn books->string [books]
  (let [book-count (count books)
    ...
    (cond
     ...
     (= book-count 1) (apply str "1 book. " book-string ".")
     :else (str book-count " books. " book-string "."))))

(defn books-by-author [author books]
  (let [validate (fn[x] (has-author? x author))]
    (filter validate books)))

(defn author-by-name [name authors]
  (let [validate (fn[x] (= (:name x) name))]
    (first (filter validate authors))))
