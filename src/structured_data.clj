...
(defn width [rectangle]
  (let [[[x1 _] [x2 _]] rectangle]
    do (if (> x1 x2) (- x1 x2) (- x2 x1))))

(defn height [rectangle]
  (let [[[_ y1] [_ y2]] rectangle]
    do (if (> y1 y2) (- y1 y2) (- y2 y1))))
...
(defn contains-point? [rectangle point]
  (let [[[x1 y1] [x2 y2]] rectangle
        [xp yp] point]
    do (if (and (<= y1 yp y2) (<= x1 xp x2)) true false)))

(defn contains-rectangle? [outer inner]
  (let [[bl tr] inner]
  do (let [blo (contains-point? outer bl)
        tro (contains-point? outer tr)]
    do (if (= blo tro) true false))))
...
(defn author->string [author]
  (let [
    ...
    do (if (contains? author :birth-year) (str nimi " (" synt " - " deat ")") (str nimi))))
