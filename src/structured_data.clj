...
(defn second-elements [collection]
  (let [secondE (fn [x] (first (rest x)))] (map secondE collection)))
...
(defn author-by-name [name authors]
  (let [authList (filter (fn [author] (= (:name author) name)) authors)]
    (if (empty? authList) nil (first authList))))

(defn living-authors [authors]
  (set (filter (fn [author] (alive? author)) authors)))

(defn has-a-living-author? [book]
  (let [authList (living-authors (:authors book))]
    (if (empty? authList) false true)))

(defn books-by-living-authors [books]
  (set (filter (fn [book] (has-a-living-author? book)) books)))
