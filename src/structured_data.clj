...
(defn element-lengths [collection]
  (let [item-len (fn [x] (count x))]
    (map item-len collection)))

(defn second-elements [collection]
  (let [item-get (fn [v] (get v 1))]
    (map item-get collection)))
...
(defn books->string [books]
  (let [num (count books)
        book_text (str num (if (== num 1) " book." " books."))]
    (if (== num 0)
      "No books."
      (str book_text " " (apply str (interpose ". " (map book->string books))) "."))))
