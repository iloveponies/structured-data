...
(defn contains-duplicates? [a-seq]
  (not (apply = (map count [a-seq (set a-seq)]))))
...
(defn author->string [author]
  (let [name (:name author)
    ...
    (cond
     dyear (str name " (" byear " - " dyear")")
     byear (str name " (" byear " - )")
     :else name)))
...
(defn books->string [books]
  (let [length (count books)]
    (if (> length 0)
      (str length " book" (if (> length 1) "s" "") ". " (apply str (interpose ", " (map book->string books))) ".")
      "No books.")))
