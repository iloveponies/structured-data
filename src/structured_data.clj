...
(defn second-elements [collection]
  (let [take-second-elem (fn [x] (get x 1))]
    (map take-second-elem collection)))
...
(defn books->string [books]
  (let [book-count (count books)
    ...
    (cond
      ...
      (= book-count 1) (str "1 book. " (first book-strings) ".")
      :else (str book-count " books. "
                        (apply str (interpose ". " book-strings)) "."))))

(defn books-by-author [author books]
  (let [has-specific-author?
          (fn [book] (has-author? book author))]
    (filter has-specific-author? books)))
