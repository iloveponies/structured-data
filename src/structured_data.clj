...
(defn author->string [author]
  (let [{name :name birth-year :birth-year death-year :death-year} author
        years (str (if birth-year
                     (str (str " (" birth-year " - ")
                     (if death-year (str death-year) "")
                     ")"))
              )]
   (str name years)
  ))
...
(defn books->string [books]
  (let [num-books (count books)
        format-book (fn [book] (str ". " (book->string book)))]
    (if (= 0 num-books)
      "No books."
      (str num-books " book" (if (= 1 num-books) "" "s")
           (apply str (map format-book books)) ".")
      )
    )
  )
